 ...
 * <p>
 * As an example, if you construct an FFT with a <code>timeSize</code> of 1024
 * and and a <code>sampleRate</code> of 44100 Hz, then the spectrum will contain
 * values for frequencies below 22010 Hz, which is the Nyquist frequency (half
 * the sample rate). If you ask for the value of band number 5, this will
 * correspond to a frequency band centered on
 ...
 * <b>Usage</b>
 * <p>
 * A typical usage of the FFT is to analyze a signal so that the frequency
 * spectrum may be represented in some way, typically with vertical lines. You
 * could do this in Processing with the following code, where <code>audio</code>
 * is an AudioSource and <code>fft</code> is an FFT.
 * 
 * <pre>
 ...
 * <b>Averages</b>
 * <p>
 * FFT also has functions that allow you to request the creation of an average
 * spectrum. An average spectrum is simply a spectrum with fewer bands than the
 * full spectrum where each average band is the average of the amplitudes of
 * some number of contiguous frequency bands in the full spectrum.
 * <p>
 ...
 * <p>
 * FFT also supports taking the inverse transform of a spectrum. This means that
 * a frequency spectrum will be transformed into a time domain signal and placed
 * in a provided sample buffer. The length of the time domain signal will be
 * <code>timeSize()</code> long. The <code>set</code> and <code>scale</code>
 * functions allow you the ability to shape the spectrum already stored in the
 * object before taking the inverse transform. You might use these to filter
...
public class FFT extends FourierTransform {
	...
	public void scaleBand(int i, float s) {
		if (s < 0) {
			System.out
					.println("Can't scale a frequency band by a negative value.");
			return;
		}
	...
	public void setBand(int i, float a) {
		if (a < 0) {
			System.out
					.println("Can't set a frequency band to a negative value.");
			return;
		}
	...
	public void forward(float[] buffer, int startAt) {
		if (buffer.length - startAt < timeSize) {
			System.out
					.println("FourierTransform.forward: not enough samples in the buffer between "
							+ startAt
							+ " and "
							+ buffer.length
	...
	public void forward(float[] buffReal, float[] buffImag) {
		if (buffReal.length != timeSize || buffImag.length != timeSize) {
			System.out
					.println("FFT.forward: The length of the passed buffers must be equal to timeSize().");
			return;
		}
	...
	public void inverse(float[] buffer) {
		if (buffer.length > real.length) {
			System.out
					.println("FFT.inverse: the passed array's length must equal FFT.timeSize().");
			return;
		}
		// conjugate
